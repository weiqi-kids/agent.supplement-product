...
| `網站改版` | 執行完整改版流程（見「網站改版流程」章節） |
| `網站健檢` | 執行 `revamp/tools/site-audit.sh {URL}` |
| `競品分析` | 執行 `revamp/tools/competitive-audit.sh {URLs}` |
...
> 📌 **適用時機**：大規模內容更新、新增主題、網站重構、品牌調整時使用。
...
請以 Reviewer 角色，參照 revamp/0-positioning/review/CLAUDE.md 檢查輸出
# ... 依序執行其他階段 ...
...
| 工具 | 用途 | 輸出 |
|------|------|------|
| `site-audit.sh` | 網站技術健檢 | Performance/SEO/Security/HTML 報告 |
| `competitive-audit.sh` | 競品比較 | 多網站效能比較表 |
...
┌─────────────────────────────────────────────────────────────────┐
│  改版流程（一次性，大規模調整）                                 │
│  revamp/ → 產出內容規格書 → 指導內容撰寫                        │
├─────────────────────────────────────────────────────────────────┤
│  例行流程（週期性，自動化）                                     │
│  Layer fetch → extract → Mode 報告 → Jekyll → 部署             │
├─────────────────────────────────────────────────────────────────┤
│  共用品質關卡                                                   │
│  階段七品質關卡 ← 技術健檢指標（site-audit.sh）                 │
└─────────────────────────────────────────────────────────────────┘
```

### Writer/Reviewer 雙角色模式

每個階段都有兩個角色確保品質：

| 角色 | 職責 | 規格文件 |
|------|------|----------|
| **Writer** | 執行分析、產出文件 | `{stage}/CLAUDE.md` |
| **Reviewer** | 檢查完整性、邏輯性、實用性 | `{stage}/review/CLAUDE.md` |

**執行流程**：
1. Writer 依據規格產出文件
2. Reviewer 檢查並回饋
3. 通過 → 進入下一階段
4. 未通過 → Writer 修正後重新送審

### 詳細文件

完整規格請參照 `revamp/CLAUDE.md`。
