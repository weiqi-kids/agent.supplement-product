...
**所有 Layer 子代理完成後只輸出一行**：
```
DONE|{layer}|F:{fetch筆數}|E:{extract筆數}|R:{review筆數}|OK
```
...
- ❌ 列舉每個 category 的檔案數
- ❌ 描述執行過程步驟
- ❌ 讀取大型 .jsonl 檔案（使用 `wc -l` + `sed -n`）
- ❌ 在回報中包含產品名稱或內容摘要

### 檔案讀取限制

- 單次 Read 最多讀取 5 個 .md 檔案
- 若需統計，使用 `find | wc -l` 而非逐一讀取
- 若需抽樣，使用 `ls | head -3` 只讀取前 3 個
